...
export async function POST(req: NextRequest) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      try {
        await sql`
          INSERT INTO licenses (key, customer_name, notes)
          VALUES (${key}, ${customer_name ?? null}, ${notes ?? null})
        `
        keys.push(key)
        inserted = true
        break
    ...
    if (!inserted) {
      return NextResponse.json({ error: 'Failed to generate a unique key, try again' }, { status: 500, headers: CORS })
    }
  }
